...
- 自定义 ADN 自己的 Android/iOS/HarmonyOS SDK；
...
2. Android 把匹配的 SDK/Adapter 依赖放到宿主 `app`；iOS 把匹配的 Pod 或
   framework 放到宿主 target；HarmonyOS 把匹配的 HAR/Adapter 放到主 module，
   并按官方要求配置 `runtimeOnly`。
3. 按官方自定义 Adapter 协议实现独立 Android/iOS/HarmonyOS Adapter 包。
...
- [HarmonyOS 自定义 Adapter](https://www.csjplatform.com/supportcenter/28678)
